...
void visual_servoing_module_init(void)
{
  visual_servoing.nominal_throttle = guidance_v_nominal_throttle;
...
{ 
  ...
  if (vs_dt > 1e-5 && !landing){
    ...
    true_distance = sqrt(pow(4 - position->x, 2) + pow(0 - position->y, 2) + pow(0.7 + position->z, 2));
